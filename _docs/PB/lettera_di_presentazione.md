---
layout: default
title: Lettera di Presentazione
permalink: docs/PB/Lettera_Presentazione.html
---


# Lettera di Presentazione PB

<a href="https://raw.githubusercontent.com/code7crusaders/docs/develop/pdf/3_PB/lettera_di_presentazione.pdf" class="github-button" download>
    <span class="github-icon">🐙</span> 
    Scarica PDF da GitHub
</a>

---

Ai professori Tullio Vardanega e Riccardo Cardin.
Con il presente documento, il gruppo *Code7Crusaders* desidera annunciare la propria intenzione di sostenere la revisione PB (*Product Baseline*) del progetto 
<div align="center">
LLM: Assistente Virtuale
</div>

proposto dall’azienda **Ergon Informatica**. La completa documentazione inerente al progetto è visibile attraverso il seguente link:

<div align="center">
 https://code7crusaders.github.io 
</div>

Nello specifico è presente una release RTB all’interno della quale sono visibili i documenti sviluppati finora, tra cui:
- **Documenti Esterni:**
  - `Analisi dei requisiti 3.0;`
  - `Piano di Progetto v2.0;`
  - `Piano di Qualifica v2.0;`
  - `Specifica Tecnica v1.0`
  - `Manuale Utente v1.0`
- **Documenti Interni:**
  - `Norme di Progetto v2.0;`
  - `Glossario v2.0;`
- **Verbali Interni:** 
  - `Verbale 25-04-04;`
  - `Verbale 25-03-21;`
  - `Verbale 24-03-14;`
  - `Verbale 25-03-07;`
  - `Verbale 25-02-28`
- **Verbali Interni:**
  - `Verbale 25-03-28;`
  - `Verbale 25-03-13;`
  - `Verbale 25-03-04;`

Di seguito viene fornito il link al repository GitHub del gruppo contenente il **Minimum Viable Product** del progetto:

<div align="center">
https://github.com/Code7Crusaders/MVP
</div>

Ad oggi il consuntivo ammonta a **€12.805**, in linea con quanto preventivato.
Nella seguente tabella sono riportate le ore produttive totali per ciascun membro del gruppo:

**Membro** | **Ore Produttive**
---|---
**Enrico Cotti Cottini** | 95
**Gabriele Di Pietro** | 95
**Tommaso Diviesti** | 95
**Francesco Lapenna** | 95
**Matthew Pan** | 95
**Eddy Pinarello** | 95
**Filippo Rizzolo** | 95
**Totale** | **665**

Di seguito vengono riportati i nomi dei componenti del gruppo **Code7Crusaders**:

**Nome** | **Matricola**
---|---
**Enrico Cotti Cottini** | 2077993
**Gabriele Di Pietro** | 2010000
**Tommaso Diviesti** | 2082858
**Francesco Lapenna** | 2072134
**Matthew Pan** | 2057869
**Eddy Pinarello** | 2075535
**Filippo Rizzolo** | 2042377


Nell'attesa di un cortese riscontro, porgiamo distinti saluti, **Code7Crusaders**
   
